var jsVersion = '?v=20160104001';

//CSS
document.write('<link rel="stylesheet" type="text/css" href="../lib/css/common.css" />\
				<link rel="shortcut icon" href="../lib/img/images/icon-webIcon.ico"/>');

//bootstrap.css
document.write('<link rel="stylesheet" type="text/css" href="../lib/plugin/bootstrap/css/bootstrap.min.css" />');
				
				
 //easyUI(datagrid, switch)
/*document.write('<link rel="stylesheet" type="text/css" href="../lib/plugin/jquery-easyui-1.4.3/themes/metro/easyui.css' + jsVersion + '" />\
				<link type="text/css" rel="stylesheet" href="../lib/css/link-braces.css"/>\
				<link rel="stylesheet" type="text/css" href="../lib/css/extend-easyUI.css' + jsVersion + '" />');
		*/
//jQueryUI(datepicker)
document.write('<link rel="stylesheet" type="text/css" href="../lib/plugin/jquery-ui-1.11.4.custom/jquery-ui.min.css' + jsVersion + '" />\
				<link rel="stylesheet" type="text/css" href="../lib/css/extend-jQueryUI.css' + jsVersion + '" />\
				<link rel="stylesheet" type="text/css" href="../lib/plugin/jquery-ui-1.11.4.custom/jquery-ui-timepicker-addon.min.css' + jsVersion + '" />');
				
//plupload
document.write('<link rel="stylesheet" type="text/css" href="../lib/plugin/plupload-2.1.2/jquery.ui.plupload/css/jquery-ui.min.css' + jsVersion + '" />\
				<link rel="stylesheet" type="text/css" href="../lib/plugin/plupload-2.1.2/jquery.ui.plupload/css/jquery.ui.plupload.css' + jsVersion + '" />');

//页面公共部分样式
document.write('<link rel="stylesheet" type="text/css" href="../index/public.css' + jsVersion + '" />\
				<link rel="stylesheet" type="text/css" href="../lib/css/default.css' + jsVersion + '" />');
//后台模版样式
document.write('<link title="" href="../lib/css/style.css" rel="stylesheet" type="text/css"/>');		

//JS
document.write('<script type="text/javascript" src="../lib/js/jquery-1.11.3.min.js' + jsVersion + '"></script>\
				<script type="text/javascript" src="../lib/js/common.js' + jsVersion + '"></script>');
				
//bootstrap
document.write('<script type="text/javascript" src="../lib/plugin/bootstrap/bootstrap.min.js"></script>');

//easyUI
//document.write('<script type="text/javascript" src="../lib/plugin/jquery-easyui-1.4.3/jquery.easyui.min.js' + jsVersion + '"></script>\
//				<script type="text/javascript" src="../lib/plugin/jquery-easyui-1.4.3/locale/easyui-lang-zh_CN.js' + jsVersion + '"></script>\
//				<script type="text/javascript" src="../lib/js/extend-easyUI.js' + jsVersion + '"></script>');

//plupload
document.write('<script type="text/javascript" src="../lib/plugin/plupload-2.1.2/jquery.ui.plupload/jquery-ui.min.js"></script>\
				<script type="text/javascript" src="../lib/plugin/plupload-2.1.2/plupload.full.min.js"></script>\
				<script type="text/javascript" src="../lib/plugin/plupload-2.1.2/jquery.ui.plupload/jquery.ui.plupload.min.js"></script>\
				<script type="text/javascript" src="../lib/plugin/plupload-2.1.2/i18n/zh_CN.js"></script>\
				<script type="text/javascript" src="../lib/js/extend-plupload.js"></script>');
			
//jQueryUI
document.write('<script type="text/javascript" src="../lib/plugin/jquery-ui-1.11.4.custom/jquery-ui.min.js"></script>\
				<script type="text/javascript" src="../lib/plugin/jquery-ui-1.11.4.custom/jquery-ui-timepicker-addon.min.js"></script>\
				<script type="text/javascript" src="../lib/plugin/jquery-ui-1.11.4.custom/i18n/datepicker-zh-CN.js"></script>\
				<script type="text/javascript" src="../lib/js/extend-jQueryUI.js"></script>');
//弹窗
document.write('<script type="text/javascript" src="../lib/js/xcConfirm.js"></script>\
		<link type="text/css" href="../lib/css/xcConfirm.css" rel="stylesheet">');			
		
//style.js 	
document.write('<script type="text/javascript" src="../lib/js/style.js"></script>');

//public.js
document.write('<script type="text/javascript" src="../index/public.js"></script>');

//durian 请求体
document.write('<script type="text/javascript" src="../lib/js/jquery.sha256.js"></script>\
				<script type="text/javascript" src="../lib/js/durian-v2.0.js"></script>\
				<script type="text/javascript" src="../lib/js/nssoft.absAPI.js"></script>');