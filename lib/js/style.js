'use strict';
//取cookie
//var userInfo = getCookie('userInfo'), test = 0;
//if (userInfo) {
//	userInfo = $.parseJSON(userInfo);
//	if (!(userInfo.user_id && userInfo.company_id && userInfo.user_level && userInfo.isDepartment && userInfo.branch_cd && userInfo.company_type)) {
...
jQuery.fn.setDialogCenter = function() {
	...
	//针对主单详情弹窗过长问题  注释下面这一行，避免点击编辑时移动位置
...
//	popup.css('margin-top', marginTop);
	...
	//判断主单详情dialog是否在显示，在显示时判断其高度，选择最大值作为遮罩
	if ($("#dialogMaster").height()) {
		height = height < $("#dialogMaster").height() ? $("#dialogMaster").height() - 30 : height;
	}
	$(this).css({
...
//	$('.defaultbox .navi').html('<a title="个人中心" href="/mypage.html" class="mypage">' + userInfo.user_id + '</a>' + 
//								'&nbsp;&nbsp;|&nbsp;&nbsp;    <a title="安全登出" href="' + contextPath + '/v1/api/user/logout.json" class="logout">安全退出</a>');
//	
	//窗口调整大小后重新fill
...
//	feedback();
	...
	$(document).on('click', '.aplPopupXbtn', function() {
		...
		$(this).parents('.popupWrapper').find('.suggest').tooltip('destroy'); //suggest验证提示遗留
	});
